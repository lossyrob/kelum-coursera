...
object Huffman {
   ...
   */
  def until(isDone: List[CodeTree] => Boolean, makeTree: List[CodeTree] => List[CodeTree])(tree: List[CodeTree]): List[CodeTree] = {
    if (isDone(tree))
      tree
    else {
      until(isDone, makeTree)(makeTree(tree))
    }
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    ...
    def decodeIter(hTree: CodeTree, bitList: List[Bit]): List[Char] = hTree match {
      case Leaf(c, w) =>
        if (bitList.isEmpty)
          List(c)
        else
  ...
  def convert(tree: CodeTree): CodeTable = tree match {
    ...
    case Fork(l, r, c, w) =>
      mergeCodeTables(concatbit(convert(l), 0), concatbit(convert(r), 1))
  }

  def concatbit(table: CodeTable, v: Int): CodeTable = {
    if (table.isEmpty)
      Nil
    else
      (table.head._1, v :: table.head._2) :: concatbit(table.tail, v)
  }
